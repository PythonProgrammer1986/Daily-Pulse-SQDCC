        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-xl font-black uppercase tracking-tight text-black">
              Strategic Focus Areas & Action Plans
            </h2>
            <p className="text-sm text-gray-500 font-medium">
              Link daily operations to strategic goals and breakthrough
              objectives
            </p>
          </div>
        </div>
        ...
        {okrs.map((okr) => {
          ...
          const progress =
            linkedTasks.length > 0
              ? Math.round(
                  linkedTasks.reduce((acc, t) => acc + t.progress, 0) /
                    linkedTasks.length,
                )
              : 0;
              ...
              <div className="p-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-1">
                    <span className="text-[10px] font-black uppercase bg-black text-[#FDB913] px-2 rounded tracking-widest">
                      OBJ-{okr.id.slice(0, 4)}
                    </span>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  {okr.keyResults.map((kr) => (
                    <div
                      key={kr.id}
                      className="p-3 bg-white border border-gray-100 rounded text-sm font-medium text-gray-700 flex items-start space-x-3"
                    >
                ...
                <div className="space-y-4">
                  ...
                  {linkedTasks.map((t) => (
                    <div
                      key={t.id}
                      className="p-3 bg-white border border-gray-100 rounded text-xs flex justify-between items-center"
                    >
          ...
          <div className="bg-white rounded shadow-2xl w-full max-w-xl overflow-hidden">
            <div className="p-6 bg-black text-[#FDB913] flex justify-between items-center">
              <h2 className="text-xl font-black uppercase tracking-tight">
                {editingOkr ? "Edit Strategy" : "Define Strategy"}
              </h2>
              <button
            ...
            <form onSubmit={handleSaveOKR} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border p-3 rounded outline-none font-bold"
                  placeholder="e.g. Increase Drill Rig Efficiency by 20%"
                />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1">
                ...
                <textarea
                  name="keyResults"
                  required
                  defaultValue={editingOkr?.keyResults
                    .map((k) => k.kr)
                    .join("\n")}
              ...
              <div className="pt-4 flex justify-end">
                <button
                  type="submit"
                  className="bg-[#FDB913] text-black px-8 py-3 rounded font-black uppercase tracking-widest shadow-xl"
                >
                  Activate Strategic Goal
                </button>
              </div>
            </form>
          </div>
