...
const HoursBooking: React.FC<HoursBookingProps> = ({
  bookings,
  tasks,
  projects,
  users,
  ...
  const getTargetName = (id: string, type: string) => {
    if (type === "task")
      return tasks.find((t) => t.id === id)?.task || "Unknown Task";
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium">
              Book actual working hours against estimated operation targets
            </p>
          </div>
        </div>
        {!readOnly && (
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {bookings.length === 0 ? (
              <tr>
                <td
                  colSpan={6}
                  className="px-6 py-10 text-center text-gray-400 uppercase font-black text-[10px]"
                >
                  No historical time logs found.
                </td>
              </tr>
            ) : (
              bookings.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50 transition group">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-xs font-black text-gray-800 line-clamp-1">
                      {getTargetName(b.targetId, b.targetType)}
                    </p>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${b.targetType === "task" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"}`}
                    >
                      {b.targetType}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm font-black text-[#FDB913]">
                    {b.hours}h
                  </td>
                  <td className="px-6 py-4 text-right">
                    {!readOnly && (
                      <button
                        onClick={() =>
                          updateBookings(bookings.filter((x) => x.id !== b.id))
                        }
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                >
                  <optgroup label="Active Projects">
                    {projects.map((p) => (
                      <option key={p.id} value={`${p.id}|project`}>
                        PRJ: {p.name}
                      </option>
                    ))}
                  </optgroup>
                  <optgroup label="Active Tasks">
                    {tasks.map((t) => (
                      <option key={t.id} value={`${t.id}|task`}>
                        TSK: {t.task}
                      </option>
                    ))}
                  </optgroup>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="e.g. 4.5"
                  className="w-full border p-2 rounded text-sm font-black"
                />
              </div>
              <div>
                <label className="block text-[10px] font-black uppercase text-gray-400 mb-1">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border p-2 rounded text-sm"
                  placeholder="Summarize activity performed..."
                ></textarea>
              </div>
              <div className="pt-4">
                <button
                  type="submit"
                  className="w-full bg-[#FDB913] text-black py-3 rounded font-black uppercase tracking-widest shadow-xl"
                >
                  Commit Hours to Ledger
                </button>
              </div>
            </form>
          </div>
